...
test('worker restart resumes a persisted opening task without a tab',async()=>{
 const h=await harness({queue:[],destinationTabId:9,job:{id:'job',startedAt:Date.now(),tasks:[task('opening',null)]}});
...
test('SPA next button can advance repeatedly when receipt content changes',async()=>{
 ...
 const h=await harness({queue:[],destinationTabId:9,job:{id:'job',startedAt:Date.now(),tasks:[task()]}},pages);
 ...
 assert.equal(h.session.job.tasks[0].count,3);assert.equal(h.session.job.tasks[0].buttons.length,2);assert.equal(h.session.job.tasks[0].state,'complete');assert.equal(h.session.queue.length,3);
});
