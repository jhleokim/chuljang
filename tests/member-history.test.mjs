...
test('transit uses actual usage date, excludes top-ups and refunds, preserves zero transfer fares and payment date differences',()=>{
 ...
 const result=parseMemberRows('tmoneyTransit',headers,[['2026-09-12',day+' 08:03','사용','서울','시청','1,500원'],['2026-09-12',day+' 08:20','사용','시청','을지로','0원'],[day,day,'충전','','','50,000원']],day);
 ...
 assert.throws(()=>parseMemberRows('tmoneyTransit',headers,[[day,'2026-09-10','사용','강남','역삼','1,500원']],day),/선택 날짜/);
 const transport=parseMemberRows('tmoneyTransit',['사용일시','교통수단','이용금액'],[[day,'지하철','1500'],[day,'편의점','5000'],[day,'버스','0']],day);assert.equal(transport.rows.length,2);
 assert.throws(()=>parseMemberRows('tmoneyTransit',['사용일시','교통수단','이용금액'],[[day,'기타','5000']],day),/교통수단/);
});
