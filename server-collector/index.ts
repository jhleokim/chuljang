...
type Packet = {version:3;automatic:true;source:string;requestedDates:string[];blocks:string[];sourceUrl:string};
type Meta = { provider?:string; consentedAt?:string; generation:number; connected:boolean; state:string; note:string; dates:string[]; jobId?:string; sessionId?:string; loginUrl?:string; loginExpires?:number; loginTarget?:string; count:number; pending:string[]; seen:string[]; runToken?:string; leaseUntil:number; startedAt:number; retry:number };
...
export class ReceiptConnection extends DurableObject<Settings> {
 ...
 async status(){const meta=await this.getMeta();return {providerId:meta.provider,connected:meta.connected,consentedAt:meta.consentedAt,state:meta.state,note:meta.note,count:meta.count,pending:meta.pending.length,requestedDates:meta.dates,loginUrl:meta.state==='login'&&(meta.loginExpires||0)>Date.now()?meta.loginUrl:undefined};}
 async start(provider:string,dates:string[],consent:boolean,jobId:string,delay:number){
  ...
  const error=await this.ctx.storage.transaction(async tx=>{
   ...
   if(meta.pending.length)return '먼저 저장 대기 영수증을 확인해 주세요.';
   if(!meta.consentedAt&&!consent)return '서비스 연결 권한을 승인해 주세요.';
   await tx.put('meta',{...meta,provider,dates:requestedDates,consentedAt:meta.consentedAt||new Date().toISOString(),generation:meta.generation+1,jobId,state:'queued',note:'서버에서 수집을 준비하고 있어요.',count:0,seen:[],retry:0,leaseUntil:0,startedAt:Date.now(),loginUrl:undefined});await tx.setAlarm(Date.now()+delay+1000);
 ...
 async stop(revoke=false){
  const previous=await this.ctx.storage.transaction(async tx=>{
   ...
   await tx.put('meta',{...meta,generation:meta.generation+1,state:revoke?'disconnected':'stopped',connected:revoke?false:meta.connected,consentedAt:revoke?undefined:meta.consentedAt,loginUrl:undefined,sessionId:undefined,leaseUntil:0,note:revoke?'연결을 해제했습니다.':'수집을 중단했습니다.',pending:revoke?[]:meta.pending});await tx.deleteAlarm();return meta;
  });
 ...
 private async verified(page:Page,provider:Provider){if(!allowedUrl(page.url(),provider))return false;return page.evaluate(()=>{const visible=(node:Element)=>!!node.getClientRects().length;const password=[...document.querySelectorAll('input[type=password]')].some(visible);const loggedIn=[...document.querySelectorAll('a,button')].some(node=>visible(node)&&/^(로그아웃|log\s*out|sign\s*out)$/i.test((node.textContent||'').trim()));return !password&&loggedIn;});}
 private async login(browser:Browser,context:BrowserContext,page:Page,provider:Provider,generation:number){
  ...
  await cdp.send('Cloudflare.handoff',{instructions:provider.name+' 공식 사이트에 로그인해 주세요. 로그인 후 완료를 누르면 선택한 출장 날짜의 내역을 수집합니다.',timeout:300000});
 ...
 async alarm(){
  ...
  try{
   ...
   if(meta.state==='login'&&!replaced){
    if(!await this.verified(page,provider)){const cdp=await context.newCDPSession(page);const handoff=await cdp.send('Cloudflare.getHandoffState',{targetId:meta.loginTarget});if(handoff.active){terminate=false;await this.patch(generation,{leaseUntil:0});await this.ctx.storage.setAlarm(Date.now()+10000);return;}}
   }
   ...
   await this.collect(page,context,provider,generation);
  }catch(error){
...
const inputSchema=z.object({owner:z.string().min(1).max(128),action:z.enum(['status','start','stop','disconnect','ack']),providers:z.array(z.string()).max(5).optional(),requestedDates:z.array(z.string()).max(62).optional(),consent:z.boolean().optional(),jobId:z.string().uuid().optional(),provider:z.string().optional(),captureId:z.string().uuid().optional(),skip:z.array(z.string().uuid()).max(200).optional()});
export default {async fetch(request:Request,env:Settings){
 ...
 try{
  if(data.action==='status'){const connections=await Promise.all(ids.map(async id=>({...await connection(id).status(),providerId:id})));const captures=(await Promise.all(ids.map(id=>connection(id).captures(data.skip||[])))).flat();return reply({configured:true,connections,captures});}
