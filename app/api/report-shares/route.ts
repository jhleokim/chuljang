...
export async function POST(request:Request){
 ...
 try{
  ...
  const id=crypto.randomUUID(),token=shareToken(),hash=await shareHash(token),key='receipts/'+crypto.randomUUID(),expiresAt=new Date(Date.now()+7*86400000).toISOString();uploaded=key;
