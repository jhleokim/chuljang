...
export default function Workspace({signedIn,homeMode=false,testMode=false,accountName='',accountId=''}:{signedIn:boolean;homeMode?:boolean;testMode?:boolean;accountName?:string;accountId?:string}) {
 ...
 return <div className="app-shell">
  ...
  <header className="topbar"><a className="brand" href="/"><span className="brand-icon"><BriefcaseBusiness size={21}/></span>출장<span className="brand-en">CHULJANG</span></a><div className="topbar-right"><span className="private-label">{testMode?'테스트 공간':'나의 정산 공간'}</span>{!testMode&&(signedIn?(homeMode?<a className="signin-link" href="/account" title={accountName+' 계정과 보안'}>계정과 보안</a>:<span className="avatar">나</span>):<a className="signin-link" href="/signin-with-chatgpt?return_to=/" target="_top">로그인</a>)}</div></header>
  ...
  {testMode&&<div className="notice">별도 가입 없이 테스트합니다. 자료는 브라우저별로 분리하며 24시간 후 삭제됩니다. 필요한 결과는 파일로 내려받으세요.</div>}
  <TripCollection signedIn={signedIn} testMode={testMode} upload={()=>openCollect()} requestedDates={requestedDates} onDatesChange={chooseDates} onProgress={setCollecting} skipCaptures={()=>[...new Set([...receivedCaptures.current,...resolvedCaptures.current])].filter(id=>id.startsWith('server:')).map(id=>id.split(':')[2])} onCapture={async(packet,id)=>{await receiveCapture(packetValue(packet),id);}}/>
  <ReportExport signedIn={signedIn} testMode={testMode} receipts={receipts} trips={trips} dates={requestedDates} collecting={collecting||busy} needsReview={drafts.length>0} />
