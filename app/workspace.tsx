...
import { ArrowDownToLine, ArrowUpRight, BriefcaseBusiness, BusFront, CarFront, Check, CircleHelp, CreditCard, FileText, Plane, Plus, TrainFront, Wallet, Upload, Puzzle, X, LoaderCircle, Pencil, AlertCircle, CalendarDays, CheckCheck } from 'lucide-react';
...
import { candidateSchema, exportCsv, parseReceipt, sourceIds, sourceNames, tripSchema, type Candidate, type Receipt, type Source, type Trip } from '@/lib/receipts';
...
type Draft=Partial<Candidate>&{source:Source;key:string;issues:string[]};
...
export default function Workspace({signedIn}:{signedIn:boolean}) {
 ...
 const [modal,setModal]=useState<'collect'|'trip'|'edit'|null>(null),[source,setSource]=useState<Source>('ktx');
 const [method,setMethod]=useState('web'),[text,setText]=useState(''),[drafts,setDrafts]=useState<Draft[]>([]),[files,setFiles]=useState<File[]>([]);
 ...
 useEffect(()=>{if(modal!=='edit'||!editing)return;let current=true;void api<{raw:string}>('/api/receipts/'+editing.id).then(data=>{if(current)setEditing(value=>value?{...value,raw:data.raw}:null);}).catch(()=>{if(current)toast.error('원문을 불러오지 못했습니다. 다시 열어 주세요.');});return ()=>{current=false;};},[modal,editing?.id]);
 ...
 function openCollect(id:Source=source){if(busy)return;setSource(id);setModal('collect');setMethod(id==='kakaot'||id==='socar'?'file':'web');}
 function parseText(){if(!text.trim()){toast.error('영수증 텍스트를 붙여넣어 주세요.');return;}setDrafts(text.split(/\n\s*---+\s*\n/).slice(0,100).map(block=>({...parseReceipt(block,source),key:crypto.randomUUID()})));setFiles([]);setImportErrors([]);}
 async function readFiles(chosen:FileList|null){
  if(!chosen?.length)return;const selected=Array.from(chosen);
  if(selected.length>20||selected.reduce((n,f)=>n+f.size,0)>12*1024*1024){toast.error('합계 12MB, 20개 이하로 선택해 주세요.');return;}
  setBusy(true);setDrafts([]);setImportErrors([]);const rows:Draft[]=[];const errors:string[]=[];
  try{for(let i=0;i<selected.length;i++){try{
   setProgress(selected[i].name+' 읽는 중…');
   const pages=await readReceiptFile(selected[i],setProgress);
   for(const page of pages){const parsed=parseReceipt(page.text,source);rows.push({...parsed,key:crypto.randomUUID(),attachmentIndex:i,issues:[...parsed.issues,...(page.warning?[page.warning]:[])]});}
  }catch(e){errors.push(selected[i].name+': '+(e as Error).message);}}
  if(rows.length>100)throw new Error('한 번에 영수증 100건까지 가능합니다.');
  setFiles(selected);setDrafts(rows);setImportErrors(errors);
  if(!rows.length&&!errors.length)toast.error('읽을 수 있는 영수증이 없습니다.');
  }catch(e){toast.error((e as Error).message);}finally{setBusy(false);setProgress('');if(inputRef.current)inputRef.current.value='';}
 }
 ...
 async function saveDrafts(){
  ...
  const parsed=drafts.map(r=>candidateSchema.safeParse(r));if(parsed.some(r=>!r.success)){toast.error('모든 영수증의 날짜·이용내역·원화 결제금액을 확인해 주세요.');return;}
  setBusy(true);try{const body=new FormData();body.set('receipts',JSON.stringify(parsed.map(r=>r.data)));files.forEach(file=>body.append('files',file));const result=await api<{imported:number;duplicates:number}>('/api/receipts',{method:'POST',body});toast.success(result.imported+'건 저장'+(result.duplicates?' · 동일 내역 '+result.duplicates+'건 제외':''));setDrafts([]);setFiles([]);setText('');setModal(null);await refresh();}catch(e){toast.error((e as Error).message);}finally{setBusy(false);}
 }
 ...
 return <div className="app-shell">
  ...
  <main className="workspace"><div className="page-heading"><div><p className="eyebrow">TRAVEL EXPENSES</p><h1>출장 영수증</h1><p className="subheading">이동은 가볍게. 영수증은 한곳에.</p></div><Button className="primary-button" onClick={()=>openCollect()}><Plus size={18}/> 영수증 수집</Button></div>
  {!signedIn&&<div className="notice">영수증을 읽고 검토할 수 있어요. 저장한 내역을 보려면 <a href="/signin-with-chatgpt?return_to=/" target="_top">로그인해 주세요.</a></div>}
  ...
  <footer><span>CHULJANG · 출장의 마지막을 가볍게</span><button onClick={()=>openCollect()}><CircleHelp size={15}/> 수집 안내</button></footer></main>
  <Dialog open={modal==='collect'} onOpenChange={open=>{if(!busy)setModal(open?'collect':null);}}><DialogContent className="collect-dialog" showCloseButton={!busy}><DialogTitle>영수증 수집</DialogTitle><DialogDescription>발급된 내역을 가져와 날짜·금액을 자동으로 정리합니다. 저장 전 내용을 확인해 주세요.</DialogDescription>
   <Select value={source} disabled={busy||drafts.length>0} onValueChange={v=>setSource(v as Source)}><SelectTrigger aria-label="수집 서비스" className="w-full"><SelectValue/></SelectTrigger><SelectContent>{providers.map(p=><SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent></Select>
   {!drafts.length&&<Tabs value={method} onValueChange={v=>{if(!busy)setMethod(v);}}><TabsList className="method-tabs"><TabsTrigger value="web"><Puzzle size={15}/> 웹 수집</TabsTrigger><TabsTrigger value="file"><Upload size={15}/> 이미지 · PDF</TabsTrigger><TabsTrigger value="text"><FileText size={15}/> 텍스트</TabsTrigger></TabsList>
   ...
   <TabsContent value="file"><label className={'upload-zone '+(busy?'is-busy':'')} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(!busy)void readFiles(e.dataTransfer.files);}}><Upload size={28}/><strong>영수증 파일을 놓아주세요</strong><span>또는 눌러서 파일 선택</span><small>JPG · PNG · WEBP · PDF / 합계 12MB</small><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" multiple disabled={busy} onChange={e=>void readFiles(e.target.files)}/></label><p className="muted upload-note">인식은 이 기기에서 처리됩니다. 저장할 때 원본과 추출 내역을 내 공간에 보관합니다. PDF는 파일당 12페이지까지 읽습니다.</p></TabsContent>
   ...
   {busy&&<div className="processing" role="status"><LoaderCircle className="animate-spin" size={19}/>{progress||'저장하는 중…'}</div>}
   {importErrors.length>0&&<div className="notice error" role="alert"><ul>{importErrors.map(s=><li key={s}>{s}</li>)}</ul></div>}
   {drafts.length>0&&<div className="draft-list"><div className="draft-title"><h3>가져온 내역 <span>{drafts.length}건</span></h3><button disabled={busy} onClick={()=>{setDrafts([]);setFiles([]);}}>다시 가져오기</button></div><p className="muted">추출값은 확인이 필요합니다. 외화·취소 내역은 실제 청구된 원화 금액으로 수정해 주세요.</p>{drafts.map((r,i)=><div className="draft-card" key={r.key}><div className="draft-header"><strong>영수증 {i+1}</strong><button aria-label={'영수증 '+(i+1)+' 가져오기 제외'} disabled={busy} onClick={()=>setDrafts(rows=>rows.filter(x=>x.key!==r.key))}><X size={17}/></button></div><div className="form-grid"><label>이용일<input type="date" value={r.date||''} disabled={busy} onChange={e=>updateDraft(r.key,{date:e.target.value})}/></label><label>결제 금액 (원)<input type="number" min={0} max={100000000} step="1" placeholder="금액 확인" value={r.amount??''} disabled={busy} onChange={e=>updateDraft(r.key,{amount:e.target.value===''?undefined:Number(e.target.value)})}/></label><label className="full">이용내역<input value={r.merchant||''} maxLength={150} disabled={busy} onChange={e=>updateDraft(r.key,{merchant:e.target.value})}/></label></div>{r.issues.length>0&&<p className="draft-warning"><AlertCircle size={14}/>{r.issues.join(' · ')}</p>}<details className="raw-details"><summary>인식한 텍스트 확인</summary><pre>{r.raw}</pre></details></div>)}<Button disabled={busy||!signedIn} onClick={()=>void saveDrafts()} className="w-full"><CheckCheck size={18}/>{drafts.length}건 저장하기</Button></div>}
   {!signedIn&&<a className="signin-link" href="/signin-with-chatgpt?return_to=/" target="_top">로그인 후 저장할 수 있어요. 이동 시 검토 중인 내역을 다시 가져와야 합니다.</a>}
