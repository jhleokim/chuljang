...
export default function Workspace({signedIn}:{signedIn:boolean}) {
 ...
 return <div className="app-shell">
  ...
  {(drafts.length>0||text.trim())&&<div className="resume-banner"><div><FileText size={21}/><span><strong>{drafts.length?drafts.length+'건 검토 중':'작성 중인 영수증이 있어요'}</strong><small>저장 전 초안은 이 화면에서만 보관됩니다</small></span></div><Button variant="outline" onClick={()=>openCollect()}>이어서 검토 <ArrowUpRight size={16}/></Button></div>}
  <TripCollection signedIn={signedIn} upload={()=>openCollect()} requestedDates={requestedDates} onDatesChange={chooseDates}/>
