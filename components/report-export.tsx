...
export function ReportExport({signedIn,receipts,trips,dates,collecting,needsReview}:{signedIn:boolean;receipts:Receipt[];trips:Trip[];dates:string[];collecting:boolean;needsReview:boolean}){
 ...
 {busy?<p role="status"><LoaderCircle className="animate-spin inline" size={16}/> {busy}</p>:collecting?<p role="status">선택한 날짜를 수집·저장하고 있어요. 완료되면 파일을 만들 수 있습니다.</p>:needsReview?<p role="status">날짜·금액이 불명확한 내역을 검토하면 파일을 만들 수 있습니다.</p>:<p className="report-hint">양식과 받는 사람은 한 번만 설정하세요. 웹메일에는 7일간 유효한 다운로드 링크를 넣습니다.</p>}
