...
async function poll() {
  ...
  try {
    ...
    post({ type: 'CHULJANG_EXTENSION', version: 3, status: result?.status });
...
window.addEventListener('message', event => {
  ...
  if (data?.type === 'CHULJANG_PROBE') { post({ type: 'CHULJANG_EXTENSION', version: 3 }); void poll(); }
  ...
  if (data?.type === 'CHULJANG_COLLECTOR_COMMAND' && ['START', 'STOP', 'OPEN_SETUP', 'OPEN_SOURCE'].includes(data.command)) {
    chrome.runtime.sendMessage({ type: data.command, providerId: data.providerId, requestedDates: data.requestedDates }).then(result => { post({ type: 'CHULJANG_COMMAND_RESULT', requestId: data.requestId, result }); void poll(); }).catch(() => post({ type: 'CHULJANG_COMMAND_RESULT', requestId: data.requestId, result: { ok: false, error: '수집 도구를 다시 연결해 주세요.' } }));
  }
});
chrome.runtime.onMessage.addListener(message => { if (message.type === 'COLLECTOR_STATUS') { post({ type: 'CHULJANG_EXTENSION', version: 3, status: message.status }); void poll(); } });
setInterval(() => void poll(), 5000);
post({ type: 'CHULJANG_BRIDGE_READY' });
post({ type: 'CHULJANG_EXTENSION', version: 3 });
