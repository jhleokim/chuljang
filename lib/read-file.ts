...
async function deadline<T>(promise:Promise<T>,ms:number):Promise<T>{let timer:ReturnType<typeof setTimeout>|undefined;try{return await Promise.race([promise,new Promise<never>((_,reject)=>{timer=setTimeout(()=>reject(new Error('인식 시간이 초과되었습니다. 더 선명한 영수증을 사용해 주세요.')),ms);})]);}finally{clearTimeout(timer);}}
async function ocr(canvas:HTMLCanvasElement,status:(s:string)=>void){
 status('한국어 영수증 인식 준비 중…');const engine=await loadEngine();let expired=false;let worker:OCRWorker|undefined;
 try{
  ...
  worker=await deadline(creating,90000);status('영수증의 글자를 읽는 중…');
  const result=await deadline(worker.recognize(canvas),90000);
...
export async function readReceiptFile(file:File,status:(s:string)=>void):Promise<{text:string;warning?:string}[]>{
 ...
 if(file.type==='application/pdf'||/\.pdf$/i.test(file.name)){
  ...
  const pdf=await task.promise;const out:{text:string;warning?:string}[]=[];
  try{if(pdf.numPages>12)throw new Error('PDF는 한 번에 12페이지까지 읽을 수 있습니다. 파일을 나누어 주세요.');
   for(let i=1;i<=pdf.numPages;i++){
    status('PDF '+i+' / '+pdf.numPages+' 페이지 읽는 중…');const page=await pdf.getPage(i);
    const content=await page.getTextContent();let text='';
    for(const item of content.items){if('str' in item)text+=item.str+('hasEOL' in item&&item.hasEOL?'\n':' ');}
    if(text.trim().length>=30){out.push({text});continue;}
    ...
    try{await page.render({canvas,viewport}).promise;const r=await ocr(canvas,status);out.push({text:r.text,warning:r.lowConfidence?'글자가 불명확합니다. 원본과 대조해 주세요.':undefined});}finally{canvas.width=canvas.height=0;}
   }
   return out;
 ...
 try{const ctx=canvas.getContext('2d');if(!ctx)throw new Error('이미지를 읽을 수 없습니다.');ctx.fillStyle='white';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.drawImage(bitmap,0,0,canvas.width,canvas.height);const r=await ocr(canvas,status);return [{text:r.text,warning:r.lowConfidence?'글자가 불명확합니다. 원본과 대조해 주세요.':undefined}];}
