...
KTX 날짜 조회·페이지 이동·원문 PDF 어댑터를 재사용합니다. 실제 회원 계정으로 최종 검증이 필요합니다. 티머니·고속버스는 `server-collector/member-history.ts`가 회원 화면의 날짜·카드·조회 버튼을 식별하고 등록 카드 및 페이지를 순회합니다. KOBUS가 결제일 기준 필터를 제공하면 화면에서 확인되는 전체 조회 기간을 검색한 뒤 이용일로 필터링합니다. 현재 실계정 조회 구조와 합계는 미검증이며, 인식하지 못하는 구조·기간·왕복 금액은 오류로 남겨 완료로 표시하지 않습니다. 티머니의 최근 이틀 내역은 미제공으로 구분하고 조회 가능해지면 다시 이어갑니다. 두 서비스의 PDF는 조회 내역 사본이며 사업자가 발급한 영수증을 대체한다고 보장하지 않습니다.
...
- 기본 XLSX 또는 업로드한 회사 XLSX 양식을 사용합니다. 시트·입력 행 범위·열 연결과 받는 사람 설정을 SQLite에 저장합니다.
- 사용자 양식의 입력 범위 밖 셀·병합·수식은 유지하고, 입력 범위에 수식이 있으면 덮어쓰지 않습니다. {{title}}, {{dates}}, {{total}}, {{count}} 치환을 지원합니다. 기존 사용자 수식은 Excel에서 열 때 재계산됩니다.
- 선택 날짜만 엑셀로 저장하거나 중복 원본을 제외한 ZIP 묶음으로 저장합니다. HWP, DOCX, XLSM은 지원하지 않습니다.
- 웹메일 공유는 7일 기한의 다운로드 링크를 만드는 방식입니다. Gmail 작성 화면을 자동으로 열고 제목·수신자·링크를 채웁니다. 메일 발송 버튼은 사용자가 누릅니다. 메일에 바이너리 파일을 자동 첨부하는 OAuth 연동은 아닙니다.
- 공유 토큰은 256비트 난수이며 DB에는 해시만 저장합니다. 계정 로그인 없이 해당 링크로만 다운로드할 수 있고 소유자가 즉시 공유를 종료할 수 있습니다. 다운로드 기한은 서버에서 강제합니다.
- 홈서버는 기존 SQLite DB를 유지하며 Drizzle SQL을 home_schema_migrations에 기록해 한 번씩 적용합니다.
