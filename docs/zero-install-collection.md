...
웹 로그인(/signin) → 출장 날짜 복수 선택 → 서비스 연결 권한 승인 → 필요한 서비스만 공식 화면에서 로그인 → 서버 수집 순서다. 확장 도구나 로컬 프로그램 설치는 필요 없다. Chrome 프로필 로그인은 서비스별 인증을 대신하지 않는다.

## 구현된 구성

- 앱의 /api/collection은 인증된 사용자 ID로 수집 서버에 요청한다. 비밀키를 브라우저에 전달하지 않는다.
...
3. 앱 환경에 CHULJANG_COLLECTOR_URL과 같은 키의 CHULJANG_COLLECTOR_SECRET을 설정한다. 비밀키는 저장소에 넣지 않는다. 로컬 개발은 무시된 .dev.vars에 설정한다.
4. 로그인 상태와 영수증 대기 내역은 AES-GCM으로 암호화하며 사용자·서비스·저장 키를 인증 데이터로 사용한다. 키 변경 시 기존 데이터가 해독되지 않으므로 연결 해제·재연결 계획이 필요하다.
5. 서버 요청은 본문·타임스탬프 HMAC으로 인증한다. 앱 API는 인증·소유자 분리·Origin 검사를 적용한다.
...
- node tests/api-smoke.mjs: 로컬 앱 API의 인증·날짜·수집 요청 검증.
